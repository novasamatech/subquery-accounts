...
const {
  ...
  resultExitCode,
  summarizeResults,
} = require('../diagnostics/compare-stg-prod');

const CHAIN_A = `0x${'a'.repeat(64)}`;
const CHAIN_B = `0x${'b'.repeat(64)}`;
const SCRIPT = path.resolve(__dirname, '../diagnostics/compare-stg-prod.js');
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (query.includes('_metadatas')) {
        ...
        body = entityResponses[req.url](query);
      } else {
...
test('parseArgs preserves values, accepts documented flags, and validates inputs', () => {
  ...
  assert.throws(() => parseArgs(['--entities=constructor']), /Unknown entities/);
  assert.throws(() => parseArgs(['--entities=__proto__']), /Unknown entities/);
...
test('schema differences affect the verdict even when global rows are unscoped', () => {
  const summary = summarizeResults({}, true, {
    accounts: { onlyStgFields: ['threshold'], onlyProdFields: [] },
  });
  assert.deepEqual(summary.schemaEntities, ['accounts']);
  assert.equal(resultExitCode({}, summary), 1);
});

test('chain exclusion fails if an event cannot be assigned to a chain', () => {
  assert.throws(() => compareEntity(['id'], side([{ id: 'event', multisigId: 'missing' }]),
    side([]), [CHAIN_A], 'operation', { stg: new Map(), prod: new Map() }), /Cannot resolve chain/);
});

test('schema probing terminates on an error outside the requested fields', async () => {
  await withFetch(async () => mockJsonResponse({ errors: [{ message: 'Cannot query field "unexpected"' }] }),
    async () => assert.rejects(probeFields('https://example.test', 'pureProxies', ['id']), /Probe failed/));
});

test('pagination rejects missing, repeated and cyclic cursors, malformed rows and incomplete results', async () => {
  const page = (nodes, hasNextPage, endCursor, totalCount = 4) => ({ nodes, totalCount, pageInfo: { hasNextPage, endCursor } });
  const cases = [
    [page([{ id: 'a' }], true, null)],
    [page([{ id: 'a' }], true, 'a'), page([{ id: 'b' }], true, 'a')],
    [page([{ id: 'a' }], true, 'a'), page([{ id: 'b' }], true, 'b'), page([{ id: 'c' }], true, 'a')],
    [page([{}], false, null, 1)],
    [page([{ id: 'a' }], false, null, 4)],
    [page([{ id: 'a' }], true, 'a'), page([], true, 'b')],
    [page([{ id: 'a' }], false, null, undefined)],
  ];
  // Explicitly unset the default count in the last case.
  delete cases[cases.length - 1][0].totalCount;
  for (const pages of cases) {
    let requests = 0;
    await withFetch(async () => {
      if (requests >= pages.length) return mockJsonResponse({ errors: [{ message: 'Requested an extra page' }] });
      return mockJsonResponse({ data: { pureProxies: pages[requests++] } });
    }, async () => assert.rejects(fetchAllEntities('https://example.test', 'pureProxies', ['id'],
      100, 'TEST', null, false, 0), /Invalid|Incomplete|Pagination/));
  }
});

test('pagination escapes opaque cursors and includes rows added during the scan', async () => {
  const cursor = 'quote"\\newline\n';
  let requests = 0;
  await withFetch(async (_endpoint, options) => {
    requests++;
    if (requests === 2) assert.ok(JSON.parse(options.body).query.includes(`after: ${JSON.stringify(cursor)}`));
    return mockJsonResponse({ data: { pureProxies: {
      totalCount: 1, nodes: [{ id: String(requests) }],
      pageInfo: { hasNextPage: requests === 1, endCursor: cursor },
    } } });
  }, async () => {
    const result = await fetchAllEntities('https://example.test', 'pureProxies', ['id'], 100, 'TEST', null, false, 0);
    assert.equal(result.total, 2);
    assert.equal(result.map.size, 2);
  });
});

test('deep JSON CLI does not report equality when schemas differ', async () => {
  const empty = { data: { pureProxies: { totalCount: 0, nodes: [], pageInfo: { hasNextPage: false, endCursor: null } } } };
  const server = await startGraphqlServer({
    '/stg': () => empty,
    '/prod': query => query.includes('entropyBlockNumber')
      ? { errors: [{ message: 'Cannot query field "entropyBlockNumber" on type "PureProxy"' }] }
      : empty,
  });
  try {
    const result = await runScript([`--stg=${server.url('/stg')}`, `--prod=${server.url('/prod')}`,
      '--entities=pureProxies', '--deep', '--json', '--no-cache', '--no-exclude-chain']);
    assert.equal(result.code, 1, result.stderr);
    assert.deepEqual(JSON.parse(result.stdout).summary.schemaEntities, ['pureProxies']);
  } finally {
    await server.close();
  }
});
